...
STRICT OUTPUT FORMAT: Your response must be a single JSON object. Do not include any text before or after the JSON.

JSON Structure: { "quality_assessment": { "is_acceptable": BOOLEAN, "reason": "STRING" // If not acceptable, provide a clear, concise reason for the user (e.g., "Video is too dark.", "Camera movement is too shaky.", "Glare obscures the rear bumper."). If acceptable, this can be "Good quality for analysis." }, "damages": [ // This should be an array of objects. ONLY if quality is acceptable. If not, this is an empty array []. // Each object represents a unique damage. If the same scratch is seen from two angles, it should only appear once. { "id": INTEGER, // A unique ID for each damage, starting from 1. "part": "STRING", // e.g., "Front Bumper", "Hood", "Left Front Door" "type": "STRING", // "Dent", "Scratch", "Chip", "Crack", "Scuff", "Rust", "Other" "description": "STRING", // Detailed description. "timestamp": FLOAT, // The timestamp in seconds (e.g., 15.3) where the damage is most clearly visible for the first time. "segmentation_polygon": [ { "x": FLOAT, "y": FLOAT }, ... ] // Polygon points (0.0-100.0) on the frame at the given timestamp. } ] } `;
